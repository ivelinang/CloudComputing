...
bool MP1Node::recvCallBack(void *env, char *data, int size ) {
	...
	case JOINREQ:
		...
		sendMemberListToMember(senderAddr, JOINREP);//sendJoinRepMsg(senderAddr);
		break;
	case JOINREP:
		memberNode->inGroup = true;
...
void MP1Node::addOrUpdateMember(Address memberAddr, long heartbeat)
{
	...
	//for each member in the existing member list
	for (vector<MemberListEntry>::iterator iter = memberNode->memberList.begin();
		iter != memberNode->memberList.end(); ++iter)
	{	//if member matches with the one from the list
		if (iter->id == id && iter->port == port)
		{
			memberFound = true;
			if (iter->heartbeat < heartbeat)	//if hearbeat is lower than new info
			{
				iter->heartbeat = heartbeat;	//update heartbeat with latest
				iter->timestamp = par->getcurrtime();	//update current time timestamp
			}
			break;
		}
	}
	//if no member in the list matches the new member
	if (!memberFound)
